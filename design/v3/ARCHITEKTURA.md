...
| 18. 8. 2026 | **P4, P5** | Produkty — bočný panel detailu | Zadanie žiada „všetky údaje vypísané", takže panel má päť blokov (predané kusy ako dominanta · prekážky · údaje o produkte · zľavy podľa vlastných zápisov · zatiaľ nedostupné) a je vyšší než obrazovka. Cena za výnimku bola merateľná: panel bol **prekryv 400 px so svojím vlastným skrolom**. Zamknuté riadky sa nedajú vynechať — z chýbajúceho riadku sa nedá zistiť, že tá informácia existuje, a práve to je dôvod, prečo panel vznikol. **Zmenené 24. 8. 2026 (šprint 20, C1):** rozhodnutie K1 spravilo z panela DRUHÝ STĹPEC vedľa tabuľky, nie prekryv — teda presne to, čo tu stálo ako podmienka pádu výnimky. Výnimka nepadla, ale jej dôvod je iný: stránka nerastie preto, že panel má `max-height: calc(100vh - 280px)` a vlastný skrol, nie preto, že leží nad obsahom. Pod ~1180 px sa zalomí pod tabuľku (prekryv na polovici monitora by zakryl celú tabuľku, nie tretinu). Nová podmienka pádu: **keby panel začal posúvať obsah stránky alebo prestal mať vlastný skrol.** Merané pri 1440/1280/1150/720 px — bez prekrytia a bez vodorovného skrolu. |
...
| ~~20. 8. 2026~~ | ~~**P2**~~ | ~~päť obrazoviek — veta o vypnutých zápisoch (`lib/status/blockers.ts`, prekážka `writes_disabled`)~~ | Veta „Zápisy do shopu sú vypnuté — appka teraz nezapíše ani jeden produkt, **nech je vo výbere čokoľvek**." má **96 znakov, teda 6 nad limitom P2**. Skracuje sa jedine odtrhnutím poslednej časti — a práve tá časť je dôvod, prečo veta existuje: bez nej používateľ prečíta „nezapíše ani jeden produkt" ako dôsledok svojho VÝBERU, zúži ho a stlačí Zaradiť znova. Vypnuté zápisy pritom nie sú vlastnosť výberu, ale konfigurácie počítača, a z obrazovky sa prepnúť nedajú (I13). Šesť znakov je menšia cena než opakovaný pokus o zápis do ostrého eshopu. Cena za výnimku je merateľná — **výnimka platí pre túto JEDNU vetu a len kým `WRITES_ENABLED=false`**. Keby sa veta rozrástla nad 96 znakov, alebo keby si niekto to isté ohnutie vypýtal pre druhú vetu, výnimka padá a veta ide pod rozklik. **ZRUŠENÁ 24. 8. 2026** — predpoklad „skracuje sa jedine odtrhnutím poslednej časti" prestal platiť. Pri skracovaní fail-closed viet (24. 8. 2026) dostalo `assumed: true` dvojča tej istej prekážky tvar „Nevieme, či sú zápisy do shopu zapnuté — appka preto nezapíše nič, **bez ohľadu na výber**." — teda ten istý dôvod povedaný kratšie, **87 znakov** meraných nad vykresleným výstupom `collectOperationBlockers()`. Ten istý tvar nad meranou vetou dáva „Zápisy do shopu sú vypnuté — appka nezapíše nič, bez ohľadu na výber." — **69 znakov, pod limitom P2**, a dôvod, prečo veta existuje (že to NIE JE o výbere používateľa), v nej zostal. Výnimka už nie je na čo. Riadok zostáva ako záznam, nie ako povolenie. |
