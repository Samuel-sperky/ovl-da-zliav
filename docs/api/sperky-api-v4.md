...
1. **API key** (required for privileged endpoints). Send it as either:
   ...
   returns `403 forbidden`. Administrators issue keys in **Tools → API keys**;
   the plaintext credential is displayed only once.
...
Exceeding either budget creates the key's configured ban (default 10 minutes)
and returns `429 rate_limited` with `Retry-After`, `retry_after`, and
`unlock_in_minutes`. Expiration, revocation, and overdue reroll deadlines make
a key invalid immediately.

## Acceptable use

Do not misuse the API or use it for automated scanning, scraping, bulk data
harvesting, or attempts to bypass rate limits and access controls. Automated
protections may ban abusive clients for up to one day. Misuse may result in a
permanent ban and revocation of the associated API key.

If you need complex or aggregated data, such as statistics, do not try to
construct it by scraping the API. It is usually safer and more efficient for us
to implement the required data access directly. Contact the maintainer at
<delaja@fedorco.sk> to discuss your requirements.
...
| 429    | `{"error":"rate_limited","unlock_in_minutes":10,...}`             | Rate limit exceeded (see `Retry-After` header)                                   |
...
## Order statistics — `/api/order-stats`

All actions require API key with scope **`order:stats`**. Aggregated/summary
data only — no per-order customer data (see Orders above for that).

**Query params (every action):** `date_from`, `date_to` (`YYYY-MM-DD`,
applied to `date_add`) — both optional, **default to today** if omitted, so
a call with neither param returns today's numbers, not all-time totals.

### `GET /api/order-stats` — every cluster below, bundled into one response

**Response:**

```json
{
 "sales": { "total_sales_tax": 138.6, "total_sales": 112.68, "total_orders": 1 },
 "margin": { "order_income": 112.68, "product_income": 111.46, "expenses": 36.08 },
 "storno": { "total_cancel": null, "total_cancel_tax": null, "total_cancels": 0 },
 "slips": { "total_slip": null, "total_slip_tax": null, "total_slips": 0 },
 "waiting_bankwire": { "total": null, "total_tax": null, "total_count": 0 },
 "waiting": { "total_bankwire": 0, "total_card": 0, "total_verification": 0 },
 "cancelled": { "total_cancel": null, "total_cancel_tax": null, "total_cancels": 0, "total_paid": null },
 "neprevzate": { "total": null, "total_tax": null, "count": 0 },
 "registrations": { "active_users": 0, "total_registrations": 0 },
 "shipped": { "total_shipping": 0, "total_shipping_vat": 0, "count": 1, "...": "..." },
 "reinjected": { "total_products_reinjected": 0, "total_products_reinjected_tax": 0 },
 "coupons": { "total_coupons_vat": null, "total_coupons": null, "total_pcs_coupons": 0 },
 "cod": { "total_cod": 1.22, "total_cod_tax": 1.5, "total_pcs_cash_on_delivery": 1, "total_pcs_gold_orders": 0 },
 "products": { "total_pcs": 7, "total_reinjected_pcs": 0, "total_expense_eur": 36.08 }
}
```

### One cluster per endpoint

Each returns just that cluster's object (the same shape as its key above in
`GET /api/order-stats`) — e.g. `GET /api/order-stats/sales` returns
`{ "total_sales_tax": 138.6, "total_sales": 112.68, "total_orders": 1 }`
directly, not wrapped.

| Endpoint                             | Cluster                                            |
| ------------------------------------ | --------------------------------------------------- |
| `GET /api/order-stats/sales`         | Order count + totals                                 |
| `GET /api/order-stats/margin`        | Margin cards (income/expenses)                       |
| `GET /api/order-stats/storno`        | Cancellation totals                                  |
| `GET /api/order-stats/slips`         | Credit note totals                                   |
| `GET /api/order-stats/waitingBankwire` | Bank-transfer orders awaiting payment              |
| `GET /api/order-stats/waiting`       | All "awaiting payment" buckets (bankwire/card/verification) |
| `GET /api/order-stats/cancelled`     | Cancelled-status order totals                        |
| `GET /api/order-stats/neprevzate`    | Not-picked-up order totals                           |
| `GET /api/order-stats/registrations` | Active users + new registrations                     |
| `GET /api/order-stats/shipped`       | Shipping totals/counts                               |
| `GET /api/order-stats/reinjected`    | Reinjected (returned) product totals                 |
| `GET /api/order-stats/coupons`       | Coupon usage totals                                  |
| `GET /api/order-stats/cod`           | Cash-on-delivery totals                              |
| `GET /api/order-stats/products`      | Sold/gifted product totals                           |

### `GET /api/order-stats/lowProfit` — orders at/under 30% margin

Not a summary row — a list of individual orders.
...
{
 "orders": [
  { "id_order": 456, "total_without_tax_eur": 40.1, "total_with_tax_eur": 48.5, "margin_percent_raw": 22.4, "...": "..." }
 ]
}
```
...
```bash
curl 'https://shop.example/api/order-stats/sales?date_from=2026-08-01&date_to=2026-08-08' \
  -H 'X-Api-Key: <your-key>'
```
...
| `order:stats`  | All `/api/order-stats/*` actions    |
